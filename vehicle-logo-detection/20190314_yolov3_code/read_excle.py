...
#     #excle数据读入部分
#     wb = xlrd.open_workbook('/home/seeking/Vehicle_Data/坐标转换.xlsx') #修改excle文件地址
#     sheet_name=wb.sheet_by_index(0)
#     lng=sheet_name.col_values(3)
#     lat=sheet_name.col_values(4)

#     # 数据处理部分
#     data_size=min(len(lng),len(lat))

#     new_lng=[]
#     new_lat=[]

#     for i in range(1,data_size):
#         temp_data=gcj02towgs84(lng[i],lat[i]) #GCJ02(火星坐标系)转GPS84
#         new_lng.append(temp_data[0])
#         new_lat.append(temp_data[1])

#     #将数据写入到excle
#     w_f=xlwt.Workbook()
#     w_sheet_name=w_f.add_sheet('sheet1',cell_overwrite_ok=True)
#     rows_name=['new_lng','new_lat']
#     for i in range(len(rows_name)):
#         w_sheet_name.write(0,i,rows_name[i])
    
#     for i in range(1,len(new_lng)):
#         w_sheet_name.write(i,0,new_lng[i-1])
#         w_sheet_name.write(i,1,new_lat[i-1])
    
#     # for i in range(1,len(new_lat)):
#     #     w_sheet_name.write(i,1,new_lat[i-1])
#     w_f.save('/home/seeking/Vehicle_Data/new_data.xlsx') #新文件保存路径
