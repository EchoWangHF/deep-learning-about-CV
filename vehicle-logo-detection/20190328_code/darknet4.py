...
class PoolTransBlock(nn.HybridBlock):
    def __init__(self,channel,pool_size,stride,model='avg'):
        super(PoolTransBlock,self).__init__()
        if model=='avg':
            self.Pool=nn.AvgPool2D(pool_size=pool_size,strides=stride)
        else:
            self.Pool=nn.MaxPool2D(pool_size=pool_size,strides=stride)
        self.conv1X1=nn.Conv2D(channel,1,1,0)
...
class DarkNet(nn.HybridBlock):
    def __init__(self, num_classes=80, input_dim=416):
        ...
        self.pooltrans_1=PoolTransBlock(64,2,4)
        self.pooltrans_2=PoolTransBlock(128,2,4)

    def hybrid_forward(self, F, x, *args, **kwargs):
        x = self.conv_bn_block_0(x)#32 416 416
        x = self.conv_bn_block_1(x)#64 208 208
        shortcut_4 = self.shortcut_block_4(x)#64 208 208
        x = self.conv_bn_block_5(shortcut_4)#128 104 104
        x = self.shortcut_block_8(x)#128 104 104
        shortcut_11 = self.shortcut_block_11(x)#128 104 104
        x = self.conv_bn_block_12(shortcut_11)#256 52 52 
        ...
        pool_trans_2=self.pooltrans_2(shortcut_11)#256 26 25
        x=F.concat(x,pool_trans_2,dim=1)
        ...
        pool_trans_1=self.pooltrans_1(shortcut_4)#128 52 52
        x=F.concat(x,pool_trans_1,dim=1) 
        ...
        for block_name in modules:
            if fine_tune:
                if block_name.find("81") != -1:
                    ...
                    continue
                elif block_name.find('pooltrans')!=-1:
                    continue
...
if __name__ == '__main__':
    # net = DarkNet()
    net=PoolTransBlock(512,2,4,max)
    net.initialize(init=mx.init.Xavier(), ctx=mx.cpu())
    net.hybridize()
    X = nd.uniform(shape=(1,256, 208, 208))
    detections = net(X)
