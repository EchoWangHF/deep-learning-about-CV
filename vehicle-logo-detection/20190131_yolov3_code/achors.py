...
parser.add_argument('--input_num_anchors',required=True,default=9,type=int,help='输入要计算的聚类（Anchor boxes的个数）')
...
if __name__=='__main__':
    ...
    annotations_w_h = []
    for name in names:
        xml_path = os.path.join(args.input_annotation_txt_dir,name)

        w,h=prase_xml(xml_path)

        annotations_w_h.append((w,h))
