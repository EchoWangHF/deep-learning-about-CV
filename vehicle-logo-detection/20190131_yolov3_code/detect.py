...
def arg_parse():
    ...
    parser.add_argument("--confidence", dest="confidence", help="Object Confidence", default=0.6, type=float)
    ...
    "params file", default="/home/seeking/Vehicle_Data/models/logo_yolov3_mxnet.params", type=str)
...
def detect_mAP():
    ...
    test_dataset = YoloDataSet(images, classes=classes, is_shuffle=True, mode="test")

    test_dataloader = gluon.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

    total_steps = int(np.ceil(len(test_dataset) / batch_size) - 1)
