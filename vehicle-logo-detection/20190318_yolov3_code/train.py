...
def arg_parse():
    ...
    parser.add_argument("--images", dest='images_path', default="/home/seeking/Vehicle_Data/data/",type=str)
    ...
    "mxnet params file", default="/home/seeking/Vehicle_Data/models/logo_focalloss_yolov3_mxnet.params", type=str)
...
class YoloDataSet(gluon.data.Dataset):
    def __init__(self, data_path,input_dim=416, is_shuffle=False, mode="train"):
        ...
        if os.path.isdir(data_path):
            ...
            image_name=os.listdir(self.image_path)

            if is_shuffle:
                shuffle(image_name)
            # self.image_list = [os.path.join(images_path, im.strip()) for im in self.image_name]
    ...
    def __getitem__(self, idx):
        ...
        label = prep_label(self.label_list[idx])
        image, label = prep_image(image, self.input_dim, label)
        label, true_xywhc = prep_final_label(label,input_dim=self.input_dim)
...
if __name__ == '__main__':
    ...
    train_dataset = YoloDataSet(args.train_data_path,is_shuffle=True, mode="train")
    ...
    if args.val_data_path:
        val_dataset = YoloDataSet(args.val_data_path,is_shuffle=True, mode="val")
    ...
    # optimizer = mx.optimizer.SGD(learning_rate=args.lr, lr_scheduler=schedule)
    ...
    for epoch in range(500):
        ...
        loss = obj_loss.get()[1] + box_loss.get()[1]
        print('sum_loss: {0}'.format(loss))

        net.save_parameters("/home/seeking/Vehicle_Data/models/logo_focalloss_yolov3_mxnet.params")
