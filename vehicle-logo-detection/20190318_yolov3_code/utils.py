...
# my_anchors=[(10, 13), (16, 30), (33, 23), (30, 61), (62, 45),
#                     (59, 119), (116, 90), (156, 198), (373, 326)]

my_anchors= [(237.37091319,180.06538895),
 ( 260.53285112 , 256.41109102),
 ( 327.21848399 , 290.22091609),
 ( 336.8214683 ,  200.57452725),
 ( 395.10612691  ,330.6821663 ),
 ( 417.45247725 , 234.25631951),
 ( 513.47169811 , 371.20754717),
 ( 561.17406143,  268.45221843),
 (1669.82758621 , 200.82758621)]
...
def predict_transform(prediction, input_dim, anchors):
    ...
    for i in range(3):
        ...
        x_y_offset = nd.repeat(nd.expand_dims(nd.repeat(nd.concat(x_offset, y_offset, dim=1), repeats=3, axis=0).reshape((-1, 2)),0),repeats=batch_size, axis=0)
        tmp_anchors = nd.repeat(nd.expand_dims(nd.repeat(nd.expand_dims(anchors[anchors_masks[i]], 0),repeats=stride * stride, axis=0).reshape((-1, 2)),0),repeats=batch_size, axis=0)
...
def my_NMS(prediction):
    ...
    for ind in range(batch_size):
        ...
        image_temp=image_pred[max_index]
        image_temp=nd.array(image_temp)
...
def write_results(prediction,confidence=0.5, nms_conf=0.5):
    ...
    for ind in range(batch_size):
        ...
        image_pred=image_pred[:,:5].asnumpy
        ...
        img_classes = np.unique(image_pred_[:, 4])
...
def parse_xml(xml_file):
    ...
    for i in root:
        ...
        if(name=='车'):
            ...
            center_y=(ymin+ymax)/2.0/image_hight
            width=box_list[2]/image_width
            height=box_list[3]/image_hight
            bbox.append([center_x,center_y,width,height])
    return np.array(bbox)

def prep_label(label_file):
    if isinstance(label_file, list):
        labels = label_file
    elif label_file.endswith(".xml"):
        labels=parse_xml(label_file)#//////////
    num_row_labels=len(labels)

    final_labels = nd.zeros(shape=(num_row_labels,5), dtype="float32")
    i = 0
    for label in labels:
        one_hot = np.zeros(shape=(5), dtype="float32")
        one_hot[4] = 1.0
        one_hot[:4] = label[:] #****************
...
def prep_final_label(labels, input_dim=416):
    ctx = labels.context
    anchors=nd.array(my_anchors,ctx=ctx)
    anchors_mask = [[6, 7, 8], [3, 4, 5], [0, 1, 2]]

    label_1 = nd.zeros(shape=(13, 13, 3, 5), dtype="float32", ctx=ctx)
    label_2 = nd.zeros(shape=(26, 26, 3, 5), dtype="float32", ctx=ctx)
    label_3 = nd.zeros(shape=(52, 52, 3, 5), dtype="float32", ctx=ctx)
    ...
    for x_box in range(labels.shape[0]):
        ...
        for i in range(3):
            ...
            true_xywhs = labels[x_box, :] * input_dim
            true_xywhs[4] = 1.0
            # print(true_label_list[i].shape)
            # print(tmp_idx[1])
            # print(tmp_idx[0])
            # print(best_anchor)
    ...
    t_y = nd.concat(label_1.reshape((-1, 5)),
                    label_2.reshape((-1, 5)),
                    label_3.reshape((-1, 5)),
                    dim=0)
